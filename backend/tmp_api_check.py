...
sample = Path('tmp_upload_test.txt')
...
body.append(f'--{boundary}\r\n'.encode())
body.append(b'Content-Disposition: form-data; name="file"; filename="tmp_upload_test.txt"\r\n')
body.append(b'Content-Type: text/plain\r\n\r\n')
body.append(sample.read_bytes())
body.append(b'\r\n')
body.append(f'--{boundary}--\r\n'.encode())
req = request.Request('http://127.0.0.1:8000/curriculum/upload', data=b''.join(body), headers={'Content-Type': f'multipart/form-data; boundary={boundary}'}, method='POST')
try:
